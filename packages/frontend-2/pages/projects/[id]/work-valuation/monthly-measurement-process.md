...
#### 1. `baseDate`、计量开始/结束时间、关联安全文明措施费均未变化

后端沿用当前已有明细，仅按前端传入的排除项和手工输入重建记录。

#### 2. `baseDate`、计量开始/结束时间或关联安全文明措施费任一变化

后端重新执行预览逻辑，按最新的计量时间段（`startDate` / `endDate`）重新计算应纳入的质量验收单及 BOQ 明细，再重建月度验工明细。

前端保存基本信息后会回刷主表，各子页监听主单的 `baseDate` / `startDate` / `endDate` / `safetyMeasureId` 变化并重新拉取数据，从而保证“月度验工”内容与最新的计量时间段一致。
