<template>
  <div>
    ...
    <!-- Page Header -->
    <div
      class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-6 mb-8"
    >
      <div>
        <h1 class="text-2xl font-bold text-slate-900">审批流程设置</h1>
        <p class="text-sm text-slate-500 mt-1">
          配置与管理项目中的各类业务审批流节点及审批人员
        </p>
      </div>
    ...
    >
      <div
        class="w-12 h-12 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center mb-3"
      >
    ...
    <div v-else class="space-y-8">
      ...
      >
        <!-- Category Title Bar -->
        <div
          class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-slate-100"
        >
          <div class="flex flex-wrap items-center gap-2.5">
            <h2 class="text-base font-bold text-slate-800">{{ cat.name }}</h2>
            <span
              class="px-2.5 py-0.5 text-xs font-semibold rounded-md bg-blue-50 text-blue-700 border border-blue-100"
            >
        ...
        <div class="mt-4">
          <div v-if="activeFlows[cat.id]" class="space-y-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-slate-700">
                  当前生效版本：{{ activeFlows[cat.id]!.name }}
                </span>
                <span
                  class="px-2 py-0.5 text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md"
                >
                  V{{ activeFlows[cat.id]!.version }}
                </span>
                <span
                  class="px-2 py-0.5 text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md"
                >
                  启用中
                </span>
              </div>
            ...
            <p class="text-xs text-slate-500 leading-relaxed">
              {{ activeFlows[cat.id]!.description || '暂无描述' }}
            </p>

            <!-- Visual Flow Diagram -->
            <div
              class="my-4 py-4 px-5 bg-slate-50/50 border border-slate-100 rounded-xl overflow-x-auto simple-scrollbar"
            >
              <div class="flex items-center gap-4 min-w-max py-1">
                <!-- Start Node -->
                <div class="flex flex-col items-center gap-2">
                  <div
                    class="w-9 h-9 rounded-full border-2 border-emerald-500 bg-white flex items-center justify-center shadow-sm"
                  >
                    <div
                      class="w-3.5 h-3.5 rounded-full bg-emerald-500 animate-pulse"
                    ></div>
                  </div>
                ...
                <template v-for="step in activeFlows[cat.id]!.steps" :key="step.id">
                  ...
                  <div class="flex flex-col items-center gap-2">
                    <div
                      class="border border-blue-100 bg-white rounded-xl shadow-sm px-4 py-2.5 flex flex-col items-center min-w-[120px] relative"
                    >
                      <div
                        class="w-7 h-7 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-1"
                      >
                      ...
                      <span
                        class="text-[10px] text-slate-400 mt-0.5 max-w-[110px] truncate"
                        :title="step.approvers.join('，')"
                      >
                        {{ step.approvers.join('，') }}
                      </span>
                      <span
                        class="text-[9px] font-semibold text-blue-600 bg-blue-50/70 border border-blue-100/50 px-1.5 py-0.5 rounded-full mt-1.5"
                      >
                ...
                <div class="flex flex-col items-center gap-2">
                  <div
                    class="w-9 h-9 rounded-full border-2 border-slate-300 bg-white flex items-center justify-center shadow-sm"
                  >
                    <svg
          ...
          <div
            v-else
            class="text-sm text-slate-400 py-6 bg-slate-50/40 rounded-2xl border border-dashed border-slate-200 text-center"
          >
            当前类别未启用任何审批流程版本
          </div>
        </div>
        ...
        <div class="mt-5 border-t border-slate-100 pt-4">
          <details class="group">
            <summary
              class="flex items-center justify-between text-[11px] font-semibold text-slate-400 cursor-pointer hover:text-slate-600 select-none transition-colors"
            >
              <span class="flex items-center gap-1">
                <svg
                  class="w-3.5 h-3.5 transition-transform group-open:rotate-90 text-slate-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <div
              class="mt-3 space-y-2.5 max-h-64 overflow-y-auto pr-1 simple-scrollbar"
            >
              <div
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="text-[11px] text-slate-600 font-semibold truncate max-w-[140px]"
                  >
                    {{ flow.name }}
                  </span>
                  <span
    ...
    >
      ...
      <div class="space-y-4 max-h-[65vh] overflow-y-auto pr-2 simple-scrollbar">
        ...
        <div
          v-if="categoryHasDefinitions && !isEditMode"
          class="text-xs text-amber-600 bg-amber-50 border border-amber-100 rounded-lg p-2.5 flex items-start gap-1.5"
        >
          <svg
          ...
          <span>
            提示：该类别在当前项目中已存在审批流程。保存新配置将生成该流程的最新版本，原启用版本将被自动替换（原审批流程实例不受影响）。
          </span>
        </div>
        ...
        <div v-if="!isEditMode">
          ...
          >
            <option value="">自定义流程 (无模板)</option>
            <option v-for="tpl in approvalTemplates" :key="tpl.id" :value="tpl.id">
              {{ tpl.name }}
            </option>
          </select>
        </div>
        ...
        <!-- Flow Name -->
        <div>
          <label for="flow-name" class="block text-xs font-bold text-slate-700 mb-1.5">
            审批流名称 *
          </label>
          <input
            id="flow-name"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <span class="block text-xs font-bold text-slate-700 mb-1.5">
              是否启用 *
            </span>
            <div class="flex items-center gap-4 mt-2">
              <label
                class="inline-flex items-center text-sm font-medium text-slate-700 cursor-pointer"
              >
                <input
                  v-model="formFlow.isActive"
                  type="radio"
                  :value="true"
        ...
        <div>
          <div
            class="flex items-center justify-between border-t border-slate-100 pt-4 mb-3"
          >
            ...
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                  <label
                    :for="`step-role-${sIndex}`"
                    class="block text-[11px] font-bold text-slate-600 mb-1.5"
                  >
                    角色/节点名 *
                  </label>
                  <input
...
const loadApprovalFlows = async () => {
  try {
    const data = await $fetch<ApprovalFlow[]>(
      `${apiOrigin}/api/projects/${projectId.value}/approval-definitions`
    )
    flows.value = data || []
  } catch (err) {
...
const deleteFlow = async () => {
  ...
  try {
    await $fetch(
      `${apiOrigin}/api/projects/${projectId.value}/approval-definitions/${selectedFlow.value.id}`,
      {
        method: 'DELETE'
      }
    )
    ...
    notify(
      '删除成功',
      `已成功删除审批流 “${originalName}” (V${version})`,
...
const toggleFlowStatus = async (flow: ApprovalFlow) => {
  ...
  try {
    await $fetch(
      `${apiOrigin}/api/projects/${projectId.value}/approval-definitions/${flow.id}/toggle-active`,
      {
        method: 'POST',
    ...
    notify(
      '操作失败',
      `无法更改审批流 “${flow.name}” 的状态`,
      ToastNotificationType.Danger
    )
  }
}

// Approval Template Definitions
...
const approvalTemplates: ApprovalTemplate[] = [
  {
    id: 'SAFETY_MEASURE_TEMPLATE_1',
    name: '安全文明措施费（模版1）',
    categoryId: 'SAFETY_MEASURE',
    description: '标准安全文明措施费审批流程模板（共8个节点，默认均为或签）',
    roles: [
      '项目经理审核',
      '安全监理审核',
      '专业工程师审核',
...
const applyTemplate = () => {
  ...
  const tpl = approvalTemplates.find((t) => t.id === selectedTemplateId.value)
  if (!tpl) return

  formFlow.value.name = tpl.name
  formFlow.value.category = tpl.categoryId
  formFlow.value.description = tpl.description
  formFlow.value.steps = tpl.roles.map((roleName, idx) => ({
...
const openCreateModal = (categoryId?: string) => {
  ...
  const targetCategory = categoryId || categories.value[0]?.id || ''
  const matchingTemplate =
    approvalTemplates.find((t) => t.categoryId === targetCategory) ||
    (targetCategory === 'SAFETY_MEASURE' ? approvalTemplates[0] : null)
  ...
  if (matchingTemplate) {
    ...
    formFlow.value = {
      ...
      category: matchingTemplate.categoryId,
      isActive: true,
      description: matchingTemplate.description,
      steps: matchingTemplate.roles.map((roleName, idx) => ({
        id: `step-${Date.now()}-${idx + 1}`,
        role: roleName,
        selectedApprovers: [],
        mode: 'OR'
