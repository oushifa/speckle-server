...
import {
  resolveExplicitFrontendOriginFromRequest,
  resolveFrontendOriginFromRequest,
  rewriteUrlToFrontendOrigin
...
export const resolveAppRedirectUrl = (req: Request, appRedirectUrl: string): URL => {
  return rewriteUrlToFrontendOrigin({
    url: appRedirectUrl,
    frontendOrigin: resolveAuthRedirectOrigin(req)
  })
}
...
export const moveAuthParamsToSessionMiddlewareFactory =
  (): RequestHandler => (req, res, next) => {
    ...
    const frontendOrigin = resolveExplicitFrontendOriginFromRequest(req)
    if (frontendOrigin) {
