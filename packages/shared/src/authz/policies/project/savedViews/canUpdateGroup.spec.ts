...
import { canUpdateSavedViewGroupPolicy } from './canUpdateGroup.js'

describe('canUpdateSavedViewGroupPolicy', () => {
  const buildSUT = (overrides?: OverridesOf<typeof canUpdateSavedViewGroupPolicy>) =>
    canUpdateSavedViewGroupPolicy({
  ...
  describe('w/ workspaced project', async () => {
    const buildWorkspacedSUT = (
      overrides?: OverridesOf<typeof canUpdateSavedViewGroupPolicy>
    ) =>
      buildSUT({
        getProject: getProjectFake({
          id: 'project-id',
